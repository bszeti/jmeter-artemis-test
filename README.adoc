...
* JMS-send.jmx: Send messages using _JMS Publisher_ with AMQP client using a _jndi.properties_ file. Run as:
  ...
  $JMETER_HOME/bin/jmeter.sh -Jthroughput=6000 -t ./JMS-send.jmx -n -l ./JMS-send.out
...
The _JMS-send.jmx_ and _JMS-receive.jmx_ scripts support the `throughput` property to limit the number of messages handled per minute.